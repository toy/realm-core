...
Parser::State* Parser::AcceptKey::end()
{
    // Document done
    bson::Bson b = std::move(parser->work.top());
    if (b.type() == bson::Bson::Type::Document) {
...
Bson Parser::parse(const std::string_view& json)
{
    using Parser = util::JSONParser;
    using EventType = Parser::EventType;

    Parser string_parser{StringData(json.data(), json.size())};
...
Parser::State* Parser::back(Bson&& b)
{
    if (work.empty()) {
        // we must be done
